...
bool TTCLocale::Parse(const std::wstring& szFilePath)
{
	Clear();
...
bool TTCLocale::Parse(BYTE* pFile, size_t size)
{
	return Parse(pFile, size, 0);
}

bool TTCLocale::Parse(BYTE* pFile, size_t size, size_t offset)
{
	Clear();
	if(pFile == NULL)
		return false;
...
bool TTCLocale::ParseFontMem(BYTE* pFile, size_t size, size_t offset)
{
...
//std::wstring TTCLocale::NarrowToWide(const std::string& szNarrow)
//{
//	return TTFLocale::NarrowToWide(szNarrow);
//}

std::string TTCLocale::WideToNarrow(const std::wstring& szWide)
{
	return TTFLocale::WideToNarrow(szWide);
}
